...
import { type ForgetCandidate, type MemoryNodeDetail, unwrap } from "@/lib/api";
...
export function MemoryDetailPanel({
  ...
  // K11-T5 (D-K11-1 / D-K11-4): a delete previews cross-persona episodic
  // evidence first; candidates open the cross-layer confirm below, an empty
  // preview falls back to the existing plain-delete confirm.
  ...
  // K11-T5 (D-K11-4): the plain graph-only delete — either the preview found
  // no episodic evidence, or the preview itself failed (best-effort; a
  // preview outage must not block deletion).
  ...
  // K11-T5 (D-K11-1): before deleting, preview the cross-persona episodic
  // evidence this node's forget would also erase. Candidates → the cross-
  // layer confirm dialog owns the rest of the flow; none → the existing
  // plain-delete confirm (D-K11-4 back-compat).
  const remove = async () => {
    if (!detail) return;
    setBusy(true);
    let candidates: ForgetCandidate[] = [];
    try {
      const preview = await unwrap(
        await api.POST("/v1/memory/nodes/{node_id}/forget-preview", {
          params: { path: { node_id: detail.id } },
        }),
      );
      candidates = preview.candidates;
    } catch {
      // Best-effort — fall through to the plain path rather than blocking
      // deletion on a preview outage.
    }
    setBusy(false);

    if (candidates.length > 0) {
      setForgetCandidates(candidates);
  ...
  // K11-T5 (D-K11-2): commit the cross-layer forget — the confirmed episodic
  // evidence (cascading its gists) AND the concept node, one operation.
  const confirmForget = async (kept: readonly ForgetCandidate[]) => {
    if (!detail) return;
    setBusy(true);
    try {
      await unwrap(
        await api.POST("/v1/memory/nodes/{node_id}/forget", {
          params: { path: { node_id: detail.id } },
          body: {
            episodic: kept.map((c) => ({
  ...
  return (
    <>
      ...
      <ForgetConfirmDialog
        ...
        busy={busy}
        onCancel={() => setForgetDialogOpen(false)}
