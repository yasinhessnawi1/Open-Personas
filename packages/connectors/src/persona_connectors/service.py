"""The connector service composition root (Spec C2 T9 / C3, extracted at Spec I1 T1).
...
**Why this is a library module and not ``__main__`` (Spec I1, D-I1-9).** The api process can
now host these runners in-process behind ``PERSONA_API_EMBED_CONNECTORS`` (D-I1-17), and
library code must never import a module named ``__main__``. So the composition lives here and
``__main__.py`` is a thin entry point that calls :func:`run_standalone`. The split is along
one seam: :func:`build_connectors` composes everything that is hosting-independent and returns
a :class:`ConnectorsBundle`; each host then decides how to RUN it. The standalone service adds
its own uvicorn on :data:`_HTTP_PORT` and gathers; the embedded api mounts the bundle's routes
into its own app and supervises the same runners in its lifespan, starting no second server
(D-I1-15).
...
class ConnectorsBundle:
    """Everything a host needs to RUN the configured connectors (Spec I1, D-I1-9).

    The hosting-independent half of the composition. :func:`build_connectors` produces
    one of these; the host decides how to run it. The standalone service adds its own
    uvicorn on :data:`_HTTP_PORT` and gathers; the embedded api mounts :attr:`http_app`
    into its own app and supervises :attr:`runners` in its lifespan, starting no second
    server (D-I1-15).

    Attributes:
        deliverers: ``platform`` to its C0 :class:`~persona.delivery.MessageDeliverer`.
            EMPTY when no platform is configured; the host decides whether that is fatal
            (standalone raises, as it always has) or merely absent (the embedded api logs
            and continues, D-I1-6).
        runners: ``name`` to a zero-argument runner FACTORY (R9-073c). A factory, never an
            already-created coroutine, because :func:`_supervised` restarts a crashed
            platform by calling it again and a bare ``Coroutine`` can only be awaited once.
            Carries the message-receiving transports only; the HTTP server is the host's
            business.
        http_app: The single merged ASGI app carrying every configured platform's HTTP
            surface (webhooks, OAuth callbacks, link-issue routes), or ``None`` when no
...
async def build_connectors(
    *,
    ...
    The extraction seam (Spec I1 T1). Every heavyweight collaborator arrives by injection
    rather than being built here, which is the whole saving the fold buys: the embedded api
    passes its OWN engines and its OWN :class:`RuntimeFactory`, so one process holds one
    model stack instead of two (``docs/ops/connectors_fold_decision.md``). The standalone
    entry point builds the same collaborators itself and passes them in, so its behaviour is
    unchanged by this refactor.

    A platform is wired iff its credentials are configured. Nothing here starts, serves or
    connects anything except the platform-identity lookups the adapters need at setup
    (Telegram ``getMe``, Discord ``/users/@me``, Slack ``auth.test``).

    Args:
        connector_config: The connector settings (tokens, transports, TTLs).
        api_config: The resolved api config, shared with the injected runtime.
        rls_engine: The RLS-scoped engine every owner-scoped read/write runs on.
        dispatch_engine: The cross-tenant engine for the pre-auth resolve/redeem reads.
        runtime_factory: The composed api runtime that produces replies.
        credits_policy: The edition's credits policy, shared with the chat-turn registry.
        job_queue: The durable enqueue surface for turn-boundary synthesis.
    ...
    # R9-081: the worker side of the conversational verbs. The runtime emits them
    # unconditionally, so without these a persona confirmed a task over Telegram, said
    # "Done, I've set that up", and nothing was created. Reschedule and initiative come from
    ...
    # A7 (T6/T8): the event-trigger emission seam, gated on PERSONA_EVENT_TRIGGERS_ENABLED.
    # A delivered inbound emits connector.message_received; a successful link emits
    # connector.linked; both dispatch through the same real dispatcher. OFF ⇒ the callbacks
    # are None and every flow is byte-identical.
    ...
    if EventTriggerSettings().enabled:
        ...
        # kill-switch store on the RLS engine) so a paused owner's triggers never fire.
    ...
    runners: dict[str, Callable[[], Coroutine[object, object, None]]] = {}
    http_apps: dict[str, FastAPI] = {}  # platform → ASGI app, merged onto one parent below
    ...
    # never collide; collect them onto ONE parent app for the host to serve or mount.
    http_app: FastAPI | None = None
    if http_apps:
        # Runtime import (FastAPI is TYPE_CHECKING-only above), kept function-local exactly
        # as the pre-extraction ``_amain`` had it: the merge is the only runtime use.
        from fastapi import FastAPI as _FastAPI

        if len(http_apps) == 1:
            http_app = next(iter(http_apps.values()))
        else:
            http_app = _FastAPI(title="persona-connectors (twilio)")
    ...
    if deliverers:
        ...
        # native-boundary connectors, so the framework store never hardcodes a platform
        # literal (D-08-3). Empty when email isn't configured ⇒ every platform swept.
        exclude_platforms = {email_adapter.PLATFORM} & set(deliverers)
        idle_sweep = functools.partial(
            _run_idle_sweep, conversation_store, idle_after, exclude_platforms
        )
...
async def run_standalone() -> None:
    """Run the connector service as its own process (the ``python -m`` path).

    Builds its own engines + :class:`RuntimeFactory` (the deploy seam), composes the bundle,
    then adds the two things that are this HOST's business and not the bundle's: the uvicorn
    server on :data:`_HTTP_PORT` carrying every platform's HTTP surface, and the idle-sweep
    task. Behaviour is unchanged from the pre-extraction ``_amain``, including raising when
    no platform is configured: standalone, that is a misconfiguration with nothing left to
    do, whereas the embedded api logs and keeps serving (D-I1-6).

    Shutdown is a normal outcome, not a fault. A cancellation of the gathered runners is
    logged as the ordinary stop it is, and the ``finally`` teardown still drains the sweep
    and closes the client. The SIGINT half is handled in :func:`persona_connectors.__main__.main`,
    because a ``KeyboardInterrupt`` raised inside a Task goes to the event loop, not to
    this coroutine.

    Raises:
        ConnectorError: No connector platform is configured.
    """
    config = ConnectorConfig()
    api_config = APIConfig()
    composition = ConnectorComposition(config)
    rls_engine = composition.make_engine()
    # R9-123 (D-I1-18): ``make_engine`` ATTACHES the role guard, but a first-connect
    # listener only fires when something first connects, which without this would be the
    # first inbound message. Force it here so a misconfigured
    # PERSONA_CONNECTORS_APP_DATABASE_URL kills the process at startup instead of being
    # discovered by a user who gets served another tenant's roster. No-op on SQLite.
    verify_rls_engine_role(rls_engine)
    ...
    # the cross-tenant dispatch engine (this process's counterpart to api's admin engine),
    ...
    try:
        bundle = await build_connectors(
            ...
            stripe_gateway=build_stripe_gateway(api_config),
            http=http,
        )
        ...
        runners = [_supervised(name, factory) for name, factory in bundle.runners.items()]
        if bundle.http_app is not None:
            runners.append(
                _supervised("http", functools.partial(_serve_app, bundle.http_app, port=_HTTP_PORT))
            )
        if bundle.idle_sweep is not None:
            sweep = asyncio.create_task(bundle.idle_sweep())
        _log.info(
            "connector service starting for: {platforms}", platforms=", ".join(bundle.deliverers)
        )
        await asyncio.gather(*runners)
    except asyncio.CancelledError:
        # An outer cancellation of the gather (the shutdown path), not a fault. The
        # KeyboardInterrupt half is caught in ``main`` instead: a KeyboardInterrupt raised
        # inside an asyncio Task escapes to the EVENT LOOP rather than to the awaiting
        # coroutine, so an ``except`` here never sees it. Proven by a test that raises the
        # real signal exception out of a real runner.
        _log.info("connector service shutting down")
    finally:
