"""Unit tests for the offline clone's missing-``git`` diagnostic (N7-T4b, R9-041 rider).

``_clone_registry`` shells out to ``git``. A runtime image missing the binary
previously failed with a bare ``FileNotFoundError`` from ``subprocess`` —
accurate but opaque. One honest WARNING now fires before the (unchanged)
attempt, naming the real cause; git-present stays byte-identical (silent,
still just the clone).
"""
...
from persona.tools.mcp.mirror_sync import _clone_registry
...
def test_missing_git_warns_before_the_still_unchanged_clone_attempt(
    ...
    # No behavior change (D-N7's fail-soft-stands rule): the attempt still
    # happens and still raises — the warning only makes the eventual failure
    # legible, it never substitutes for or swallows it.
    with pytest.raises(FileNotFoundError):
        _clone_registry("https://example.invalid/mcp-registry.git", tmp_path / "dest")
