...
Verifies: bytes land on disk at the right workspace path with the right F5
sidecar (per-format producing_spec/type mapping); failure modes each raise
:class:`FileExtractionError` with the documented fixed-vocabulary reason; the
sandbox session is ALWAYS released; and the session is ISOLATED from the live
chat conversation's own session id.
"""
...
async def test_success_persists_bytes_and_writes_the_f5_sidecar(tmp_path: Path) -> None:
    ...
    assert result.workspace_path == "uploads/board-game-report.pdf"
    assert result.media_type == "application/pdf"
    assert result.size_bytes == 9
    assert result.format == "pdf"

    target = tmp_path / "owner_1" / "persona_1" / "uploads" / "board-game-report.pdf"
...
async def test_producing_spec_and_sidecar_type_per_format(
    ...
    target = tmp_path / "owner_1" / "persona_1" / "uploads" / ref
    meta = read_artifact_sidecar(target)
    assert meta is not None
