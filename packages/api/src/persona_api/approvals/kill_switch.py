...
class KillSwitchStore:
    ...
    def __init__(self, engine: Engine, *, continuation: TaskContinuation | None = None) -> None:
        """``continuation`` is required only for :meth:`cancel_task`. The origination gates
        (A5/A7/A10) and the leg-runner guard consult the read-only pause/suspend predicates
        (``is_owner_autonomy_paused`` / ``is_persona_suspended`` / ``is_runnable``) and may build
        the store without one — the A6-D-8 completeness seam wants exactly that read.
        """
        self._engine = engine
    ...
    def cancel_task(self, owner_id: str, task_id: str, *, now: datetime) -> CancellationSummary:
        ...
        if self._continuation is None:  # pragma: no cover — a read-only store never cancels
            raise RuntimeError("cancel_task requires a TaskContinuation; this store is read-only")
