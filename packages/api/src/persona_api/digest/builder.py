...
main sections never depend on it (A6-D-10). The A7 "ran because" line is flag-gated (no data source
until A7 merges; ``ran_because`` stays ``None``).
"""
...
def _section(kind: str, items: list[DigestItem]) -> DigestSection | None:
    ...
    return DigestSection(
        kind=kind, items=tuple(items[:cap]), overflow=max(0, len(items) - cap)
    )
...
def build_morning_digest(
    ...
    *,
    ...
    spent = 0
    for task in reader.list_recent_terminal(limit=_TERMINAL_LIMIT):
...
def _resolve_names(
    ...
    with rls_connection(engine, owner_id) as conn:
        rows = (
            conn.execute(
                select(personas_t.c.id, personas_t.c.yaml).where(personas_t.c.id.in_(ids))
            )
            .mappings()
            .all()
        )
